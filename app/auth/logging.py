...
async def log_admin_activity(
    ...
    try:
        # 로거로 기록
        logger.info(
            f"[ADMIN_LOG] {severity} - Admin {admin_id}: {action} - {description}"
        )

        # 중요한 활동은 별도 경고 로그
        if severity in ["HIGH", "CRITICAL"]:
            logger.warning(
                f"[중요 관리자 활동] {description} (관리자: {admin_id}, 심각도: {severity})"
            )
        ...
        if db:
            try:
                admin_log = SystemLog(
                    ...
                    message=f"Admin {admin_id}: {action} - {description}",
                    context={
                        "admin_id": admin_id,
                        "action": action,
...
class AdminLogService:
    ...
    def get_recent_activities(
        ...
        try:
            ...
            if admin_id:
                query = query.filter(
                    SystemLog.context.op("->>")("admin_id") == str(admin_id)
                )
    ...
    def get_activity_statistics(self) -> dict:
        ...
        try:
            ...
            today_activities = (
                self.db.query(func.count(SystemLog.log_id))
                .filter(SystemLog.created_at >= today)
                .scalar()
                or 0
            )
            ...
            # 심각도별 통계
            severity_stats = (
                self.db.query(SystemLog.level, func.count(SystemLog.log_id))
            ...
            return {
                ...
                "total_logs": self.db.query(func.count(SystemLog.log_id)).scalar() or 0,
            }
