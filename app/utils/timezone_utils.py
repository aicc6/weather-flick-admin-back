...
class TimezoneUtils:
    ...
    @classmethod
    def get_admin_log_timestamp(cls) -> datetime:
        """관리자 로그용 UTC 타임스탬프 반환"""
        return cls.now_utc()
    
    @classmethod
    def format_batch_job_time(cls, dt: datetime) -> str:
        """배치 작업 시간을 API 응답용으로 포맷"""
        return cls.format_for_api(dt)
...
def admin_log_timestamp() -> datetime:
    """관리자 로그용 UTC 타임스탬프"""
    return TimezoneUtils.get_admin_log_timestamp()
