...
from ..models import Admin, User, AdminActivityLog, WeatherData
...
class DashboardService:
    ...
    async def _get_user_statistics(
        ...
        try:
            ...
            return {
                ...
                "growth_rate_week": (
                    round((new_week / total_users * 100), 2) if total_users > 0 else 0
                ),
                "verification_rate": (
    ...
    async def _get_admin_statistics(
        ...
        try:
            ...
            # 주간 활동한 관리자 수 (로그 기준)
            active_week_admins = (
                self.db.query(func.count(func.distinct(AdminActivityLog.admin_id)))
                .filter(AdminActivityLog.created_at >= week_ago)
                .scalar()
                or 0
            )
            ...
            return {
                ...
                "activity_rate": (
                    round((active_week_admins / active_admins * 100), 2)
                    if active_admins > 0
                    else 0
                ),
            }
    ...
    async def _get_system_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "latest_weather_time": (
                    latest_weather.isoformat() if latest_weather else None
                ),
