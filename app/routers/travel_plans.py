...
@router.get("/", response_model=PaginatedResponse[TravelPlanResponse])
def list_travel_plans(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    plans, total = service.get_travel_plans_with_count(db, skip=skip, limit=limit)
    result = []
    for plan in plans:
    ...
    return PaginatedResponse[TravelPlanResponse](
        items=result,
        total=total,
        page=page,
        size=limit
